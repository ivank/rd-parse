import { Ignore, All, Any, Optional, Star, Node, Y, FunctionRule, Rule } from '../src';
...
export const IgnoreWhitespace = (Rule: FunctionRule) => Ignore(/^\s+/, Rule);
...
const Grammar = Y(function (Expression) {
  const Identifier = Node(IdentifierToken, ([name], $) =>
    Object.defineProperty({ type: 'Identifier', name }, 'pos', { writable: true, configurable: true, value: $.pos }),
  );

  // Literals
  const StringLiteral = Node(StringToken, ([raw]) => ({ type: 'Literal', value: eval(raw as string), raw }));
  ...
  const TemplateLiteral = Node(All('`', Star(Any(InterpolationChunk, TemplateInlineExpression)), '`'), (parts) => ({
  ...
  const PropertyDefinition = Node(Any(All(PropertyName, ':', Expression), Identifier), ([name, value]) => ({
    name,
    value: value || name,
  }));
  ...
  const MemberExpression = Node(All(PrimaryExpression, Star(Any(PropertyAccess, Arguments))), (parts) =>
  ...
  const Operator = (Rule: Rule) => Node(Rule, (_, $, $next) => $.text.substring($.pos, $next.pos));
  ...
  const associativity = (BinaryOp: Rule) => (BinaryOp === '**' ? rightToLeft : leftToRight);

  function leftToRight(parts: any[]) {
    let left = parts[0];
  ...
  function rightToLeft(parts: any[]) {
  ...
  const ExpressionConstructor = (Expr: Rule, BinaryOp: Rule) =>
    Node(All(Expr, Star(All(Operator(BinaryOp), Expr))), associativity(BinaryOp));
  ...
  const BindingElement = Node(
    All(Identifier, Optional(All('=', Expression))), // Do not support destructuring just yet
  ...
  const ArrowParameters = Node(Any(Identifier, FormalParameters), ([params]) =>
    params.bound ? params : { bound: [params] },
  );
